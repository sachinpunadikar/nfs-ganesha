...
class DumpFSALStats():
	...
	if not self.stats[0]:
	    ...
	    output += "    Op-Name \t     TotalOps   Res:Avg      Min         Max \n"
	    i = 0
	    tot_len = len(self.stats[3])
	    while (i+5) < tot_len:
		output += "\n" + (self.stats[3][i+0]).ljust(20)
		output += "%s" % (str(self.stats[3][i+1]).rjust(8))
		output += " %11.5f" % (self.stats[3][i+2])
		output += " %11.5f" % (self.stats[3][i+3])
		output += " %11.5f" % (self.stats[3][i+4])
